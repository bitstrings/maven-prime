...
A goal is a whole build, not a string: goals, profiles, properties, options, JRE and the Maven that runs it.
Keep it in the global list to get it in every project, or in the project list to keep it with the project.

Every goal is also a real IDE action. It shows up in **Settings → Keymap** and in the toolbar settings, so a
goal you run constantly can have its own shortcut. Renaming it keeps every binding you made.
...
The module comes from whatever you point at, and the status bar shows which one. Running from the editor or
the gutter creates an ordinary run configuration, so it is rerunnable and editable like any other.
...
read live, so pulling a teammate's change is the whole of the change.
...
Results come from the Surefire reports the build actually wrote, so a run through Maven fills the test tree the
way a run through the IDE does. Debug attaches to the JVMs Surefire forks, so breakpoints are hit instead of
being skipped in a child process.

## In the POM editor

- **Effective values, inline.** A `${property}` in a version shows what it resolved to, and a dependency with
  no version shows the one it inherits. Clicking the value opens the file and line that declares it.
- **Dependency Analyzer** tab on every `pom.xml`.
- **Intentions** to analyze a dependency, go to the declaration that sets its version, or exclude a transitive.
...
- **Sequential**: nothing ran in parallel.
...
dependency, build plugin and property of the module, beside the POM that actually set it. Double-click goes to
that file at that line, including parents that only exist in `~/.m2`. Plugins you never declared are listed
too, with the model that injected them.

**Refresh Model** fills the tab without a build. **Auto-Refresh** re-reads it whenever a change makes the
effective model change underneath you.

## Repository

`Could not transfer artifact ... will not be reattempted until the update interval has elapsed` is a sentence
about files on disk that nothing reads for you. The **Repository** tab reads them, and only touches the network
when you ask.

Type `commons-lang3` to find it; full coordinates are optional. You can search the local repository, the IDE's
index of everything published, the module's dependencies or plugins, or what a build actually downloaded and
failed to resolve.
...
- **Exclude**: writes the `<exclusion>` into the declaration that brings it in, and tells you when that
  declaration lives in a parent
...
IntelliJ IDEA 2024.3 or newer, Community or Ultimate. Maven comes from the IDE. `mvnd` is optional and only
needed for daemon builds.

The Profiler, Model and Repository tabs are filled by a Maven Prime build on either engine, since they report
what Maven did rather than what the IDE assumes.

## Installation

- **From JetBrains Marketplace**: <kbd>Settings</kbd> → <kbd>Plugins</kbd> → <kbd>Marketplace</kbd> → search
  for <kbd>Maven Prime</kbd> → <kbd>Install</kbd>.
- **From disk**: <kbd>Settings</kbd> → <kbd>Plugins</kbd> → <kbd>⚙️</kbd> →
  <kbd>Install Plugin from Disk…</kbd>, and pick the archive from
  [the latest release](https://github.com/bitstrings/maven-plus/releases/latest).
...
[Report an issue](https://github.com/bitstrings/maven-plus/issues) · Apache License 2.0
